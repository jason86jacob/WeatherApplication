...
extension WeatherDetailsViewModel {
    ...
    func fetchWeatherDetails(cityName:String? = nil, stateCode:String? = nil, countryCode:String? = nil, completionHandler: @escaping (() -> Void)) {
        ...
        // Validate bussiness logic to check if user has inputted City Name OR (city name + state code + country code) together
        if weatherInputValidationStatusPassed(cityName: cityName ?? "", stateCode: stateCode ?? "", countryCode: countryCode ?? "") {
            let finalUrlString = Constants.API.weatherAPIwithImperialMetrics + queryString
            // Pass the final url string to processWeatherDataforURL which can be reused for both types of weather api calls(search based and location based)
        ...
        locationManager.$coordinates.sink { [weak self] coordinates in
            if coordinates?.status ?? false,
               ...
               let longitude = coordinates?.longitude {
                // Once coordniates are published with proper value, create the final url for weather API
                let finalUrlString = Constants.API.weatherAPIwithImperialMetrics + Constants.API.latitudeQuery + "\(latitude)" + Constants.API.ampersand + Constants.API.longitudeQuery + "\(longitude)"
                // Pass the final url string to processWeatherDataforURL which can be reused for both types of weather api calls(search based and location based)
                self?.processWeatherDataforURL(finalUrlString, completionHandler: completionHandler)
            } else {
                // Check the location permissions and see if status is turned off, if yes then pass on the control to let view controller know about it so that it can try to fetch weather api using last saved city name
    ...
    // MARK: Reusable API caller which will get the url as input string and parse the response for view controller to use.
            ...
            case .success(let data):
                do {
                    ...
                    DispatchQueue.global().async {
                        // Check if the image is available in our ImageCacheManager singleton object and the key is the icon name that comes down from the first weather api
                        if let imageIconData = self?.retrieveFromImageCacheForKey(parsedWeatherData.weather[0].icon) {
                            // We update the published "iconImageData" object so that viewcontroller knows to make use of it through data binding. Just showcasing a mix of both completionHandler and data binding approaches
                            ...
                            self?.webserviceManager.callWebservice(urlString: iconURL) {[weak self] result in
                                ...
                                case .success(let data):
                                    ...
                                    // We update the published "iconImageData" object so that viewcontroller knows to make use of it through data binding.
                                    self?.iconImageData = self?.retrieveFromImageCacheForKey(parsedWeatherData.weather[0].icon)
                                    self?.constructDetailsForDisplay(parsedWeatherData)
                                    completionHandler()
                                }
                            }
...
extension WeatherDetailsViewModel {
    // Business loggic to check if city Name OR cityName + state code + country code combination was provided by user on search
    func weatherInputValidationStatusPassed(cityName:String, stateCode:String, countryCode:String) -> Bool {
        ...
        if (!cityName.isEmpty && stateCode.isEmpty && countryCode.isEmpty) || (!cityName.isEmpty && !stateCode.isEmpty && !countryCode.isEmpty) {
            return true
        }
        return false
    }
...
extension WeatherDetailsViewModel {
    ...
    func constructDetailsForDisplay(_ weatherData: WeatherModel? = nil) {
        ...
        guard let weatherData = weatherData else {
            // No weather model data from response, mainly intented for use cases when location permissions are disabled,
            // so that we can make an attempt to fetch weather details for last saved city.
        ...
        currentWeatherData[Constants.WeatherData.temperature] = "\(weatherData.mainStats.temp)" + Constants.API.degreeSymbol + Constants.Common.farenheitUnit
        currentWeatherData[Constants.WeatherData.description] = weatherData.weather[0].description
        currentWeatherData[Constants.WeatherData.highTemperature] = Constants.WeatherData.highTemperatureLabelText + "\(weatherData.mainStats.tempMax)" + Constants.API.degreeSymbol + Constants.Common.farenheitUnit
        currentWeatherData[Constants.WeatherData.lowTemperature] = Constants.WeatherData.lowTemperatureLabelText + "\(weatherData.mainStats.tempMin)" + Constants.API.degreeSymbol + Constants.Common.farenheitUnit
        currentWeatherData[Constants.WeatherData.feelsLike] = "\(weatherData.mainStats.feelsLike)" + Constants.API.degreeSymbol + Constants.Common.farenheitUnit
        currentWeatherData[Constants.WeatherData.pressure] = "\(weatherData.mainStats.pressure)" + Constants.Common.pressureUnit
        currentWeatherData[Constants.WeatherData.wind] = "\(weatherData.windConditions.speed)" + Constants.Common.windSpeedUnit
    }
}
