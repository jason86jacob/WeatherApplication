...
class LocationManager: NSObject, CLLocationManagerDelegate {
    ...
    func locationManagerDidChangeAuthorization(_ manager: CLLocationManager) {
        if [CLAuthorizationStatus.authorizedAlways,CLAuthorizationStatus.authorizedWhenInUse].contains(manager.authorizationStatus) {
            // We are requesting location data only once so making use of requestLocation method
            locationManager.requestLocation()
        } else {
            print("location preferences doesn't have always and in use permission") //Use for remote logging like splunk
        }
    }
    
    // MARK: Convinience method to check for location service authorization status
    func locationPermissionStatus() -> Bool {
        if [CLAuthorizationStatus.authorizedAlways,CLAuthorizationStatus.authorizedWhenInUse].contains(locationManager.authorizationStatus) {
            return true
        }
        return false
    }
